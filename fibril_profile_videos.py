...
for ii in range(len(file_search(fibdir,'crispex*3950*.csav'))):
    ...
    for oo in range(len(file_search(outdir, "cut"+cut_file[-11:-5]+"*txt"))):
        ...
        #ax0.set_title(tit, fontdict = font, loc = 'left')
        ax0.set_title(osc_fname[31:-4], fontdict = font, loc = 'left')
        ...
        #if (oo==1 or oo==3):
        ax1.set_ylabel(r"$\hat{C}$", fontdict = font)
        ...
        #if (oo==1 or oo==3):
        ax2.set_ylabel(r"$\hat{R}$", fontdict = font)
        ...
        osc_fname_plot = outdir+"cut_profiles/osc"+str(ii).zfill(2)+str(oo).zfill(2)+".pdf"
        #fig.savefig(osc_fname_plot, quality = 100)
        fig.savefig(outdir + "cut_profiles/" + osc_fname[31:-4]+".pdf", quality = 100) #print("file saved to: "+osc_fname_plot)
        #print(cut_file, osc_fname[31:-4])
        plt.close("all")
        ...
        if(0):
            ...
            f3 = plt.figure(figsize = (5,7))#max(t)*8./75.))
            ...
            if (oo==6):
            '''
            plt.legend(loc = 'upper center', ncol = 3, fontsize = 12., handlelength = 1.5)
            ...
            f3.savefig(outdir +"cut_profiles/3d"+str(ii).zfill(2)+str(oo).zfill(2)+".pdf", quality = 100)
            plt.close(f3)
